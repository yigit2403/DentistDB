...
- Installer: a wizard page to choose the network port (default 5000; an upgrade offers the previously configured port).
- CI workflow: every push and pull request builds the app and the import tool and runs the test suite on a Windows runner.
...
- Treatment record search was only case-insensitive for ASCII, so "çekim" did not find "Çekim"; records now carry a Turkish-folded search index (backfilled at startup) and the search uses it.
- Treatment record form: an unknown patient id crashed with a 500 instead of a validation message, dates in the future were accepted, and typed tooth tokens such as "abc" or "99" were stored; all three are validated now.
